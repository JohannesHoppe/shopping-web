(function webpackUniversalModuleDefinition(root, factory) {
	if(typeof exports === 'object' && typeof module === 'object')
		...
		exports["multiagent"] = factory();
	else
		root["multiagent"] = factory();
})(this, function() {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var SimpleFailover = __webpack_require__(8);
	var ConsulFailover = __webpack_require__(9);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var FormData = __webpack_require__(6); // browserify compatible
...
/***/ function(module, exports) {
	...
	var SimpleFailover = function SimpleFailover(options) {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var ConsulFailover = function ConsulFailover(options) {
	...
	ConsulFailover.prototype.resolveServers = function (cb) {
	  ...
	  var stillValid = this._serverList.length && Date.now() - this._lastDiscovery < this.refreshAfter;
	  ...
	  var req = new Request('GET', this._discoveryRequestPath, this._discoveryFailOver);
	  req.timeout(this.discoveryTimeout);
	  req.end(function (err, res) {
	    var servers = !err ? _this.createServerListFromConsulResponse(res.body, _this.serviceProtocol) : [];
	    var error = err || (!servers.length ? new Error('No endpoints found for service \'' + _this.serviceName + '\'') : null);
	    var callbacks = _this._callbacks;
	    _this._lastDiscovery = Date.now();
	    _this._isDiscovering = false;
	    _this._serverList = servers;
	    _this._callbacks = [];
	...
	ConsulFailover.prototype.createServerListFromConsulResponse = function (body, serviceProtocol) {
	  return body.map(function (x) {
	    return serviceProtocol + '://' + x.Service.Address + ':' + x.Service.Port;
	  });
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var arrayShuffle = __webpack_require__(11);
	var superagent = __webpack_require__(1);

	var Request = function Request(method, path, failOver) {
	  this.method = method;
	  this.path = path;
	...
	['set', 'query', 'send', 'type', 'accept', 'timeout', 'auth', 'redirects', 'attach', 'field', 'withCredentials'].forEach(function (key) {
	...
	Request.prototype.end = function (cb) {
	  ...
	  if (this._failOver) {
	    this._failOver.resolveServers(function (err, addr) {
	      if (err) return cb && cb(err);
	      if (!addr || !addr.length) return cb && cb(new Error('server list is empty'));
	      var urls = addr.map(function (server) {
	        return concatPath(server, _this.path);
	      });
	      if (_this._failOver.strategy === 'simultaneously') {
	        _this._executeSimultaneously(urls, cb);
	      } else if (_this._failOver.strategy === 'sequentially') {
	        _this._executeSequentially(urls, cb);
	      } else if (_this._failOver.strategy === 'randomly') {
	        _this._executeSequentially(arrayShuffle(urls), cb);
	      } else {
	        return cb && cb(new Error('Unknown strategy \'' + _this._failOver.strategy + '\''));
	      }
	    });
	  } else {
	  ...
	  this._promise = this._promise || new Promise(function (resolve, reject) {
	    _this2.end(function (err, res) {
	      if (typeof res !== 'undefined' && res.status >= 400) {
	        var msg = 'Cannot ' + _this2.method + ' ' + _this2.url + ' (' + res.status + ')';
	...
	Request.prototype._executeSingle = function (url, cb) {
	  var req = superagent(this.method, url);
	  this._actions.forEach(function (x) {
	    return req[x.key].apply(req, x.params);
	  });
	...
	Request.prototype._executeSimultaneously = function (urls, cb) {
	  ...
	  urls.forEach(function (url) {
	    var req = _this3._executeSingle(url, function (err, res) {
	      remaining -= 1;
	      if (settled) return;
	      if (!_this3._failOver.shouldFailOver(err, res, _this3) || remaining === 0) {
	        settled = true;
	        if (cb) cb(err, res);
	        _this3._requests.filter(function (x) {
	          return x !== req;
	        }).forEach(function (x) {
	          return x.abort();
	        });
	...
	Request.prototype._executeSequentially = function (urls, cb) {
	  ...
	  var url = urls[0];
	  this._executeSingle(url, function (err, res) {
	    if (!err || !_this4._failOver.shouldFailOver(err, res, _this4)) return cb && cb(err, res);
	    if (urls.length <= 1) return cb && cb(err || new Error('No more fail over URLs to try'));
	    return _this4._executeSequentially(urls.slice(1), cb);
	  });
	};
...
/***/ function(module, exports, __webpack_require__) {
	...
	var Client = function Client(failover) {
	  this._failover = failover;
	};
	...
	['head', 'get', 'post', 'put'].forEach(function (key) {
	  Client.prototype[key] = function (path, data, fn) {
	    var req = this.request(key.toUpperCase(), path);
	...
	['delete', 'del'].forEach(function (key) {
	  Client.prototype[key] = function (path, fn) {
	    var req = this.request('DELETE', path);
	    if (fn) req.end(fn);
	    return req;
	  };
	});
